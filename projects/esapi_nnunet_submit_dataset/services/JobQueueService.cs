    ...
    public class JobQueueService
    {
        ...
        public JobQueueService(string queueDirectory = null)
        {
            ...
            if (!Directory.Exists(_queueDirectory))
            {
                ...
                {
                    Console.Error.WriteLine($"ERROR: Queue directory path not found: {_queueDirectory}");
                    Console.Error.WriteLine("Please check that the 'app_data_dir' path in config.json is valid and accessible.");
                }
                catch (IOException ex)
                {
                    Console.Error.WriteLine($"ERROR: Cannot create queue directory: {_queueDirectory}");
