...
namespace VMS.TPS
{
  ...
  public class Script
  {
    ...
    public void Execute(ScriptContext context, System.Windows.Window window)
    {
      ...
      m_structureSet = context.StructureSet;
    ...
    private void InitVisualizationElements(DockPanel layoutRoot)
    {
      ...
      // Ink canvas for contouring strokes
      {
        m_inkCanvas = new InkCanvas();
        m_inkCanvas.Width = m_image.XSize;
        m_inkCanvas.Height = m_image.YSize;
        m_inkCanvas.Background = null;
        m_inkCanvas.StrokeCollected += new InkCanvasStrokeCollectedEventHandler(OnInkCanvasStrokeCollected);
        m_inkCanvas.UseCustomCursor = true;
        m_inkCanvas.Cursor = Cursors.Pen;
        m_canvas.Children.Add(m_inkCanvas);
      }
    }
    ...
    private void InitControlsPanel(StructureSet ss, Panel layoutRoot)
    {
      ...
      // Structure selection combo and button for creating a new 'margin structure'
      {
        m_structuresCombo = new ComboBox();
        m_structuresCombo.ItemsSource = ss != null ? ss.Structures : null;
        m_structuresCombo.SelectionChanged += new SelectionChangedEventHandler(OnStructuresComboSelectionChanged);
        m_structuresCombo.MinWidth = 120.0;

        Button btn = new Button();
        btn.Content = "Add 5mm Margin";
        btn.Click += new RoutedEventHandler(OnMarginButtonClick);

        StackPanel panel = new StackPanel();
        panel.Children.Add(m_structuresCombo);
    ...
    //---------------------------------------------------------------------------------------------
    void OnMarginButtonClick(object sender, RoutedEventArgs e)
    {
      if (m_structureSet != null && m_structure != null)
      {
        int n = m_structureSet.Structures.Where(s => s.Id.StartsWith("ORGAN")).Count();
        Structure newStructure = m_structureSet.AddStructure("ORGAN", "ORGAN" + (n + 1).ToString());
        newStructure.SegmentVolume = m_structure.Margin(5.0);
        m_structuresCombo.ItemsSource = m_structureSet.Structures;
        m_structuresCombo.SelectedItem = newStructure;
        Render();
      }
    }
    ...
    //---------------------------------------------------------------------------------------------
    void OnInkCanvasStrokeCollected(object sender, InkCanvasStrokeCollectedEventArgs e)
    {
      if (m_structure != null)
      {
        int points = e.Stroke.StylusPoints.Count;
        VVector[] contour = new VVector[points];
        int i = 0;
        foreach (var sp in e.Stroke.StylusPoints)
        {
          contour[i++] = m_imageRender.MapToDICOM(sp.ToPoint());
        }
        if (Keyboard.IsKeyDown(Key.LeftShift) || Keyboard.IsKeyDown(Key.RightShift))
        {
          m_structure.SubtractContourOnImagePlane(contour, m_imageRender.CurSlice);
        }
        else
        {
        m_structure.AddContourOnImagePlane(contour, m_imageRender.CurSlice);
        }
        Render();
      }
    }
    ...
    void UpdateInkCanvas()
    {
      ...
      if (m_structure != null && m_structure.HasSegment)
      {
        m_inkCanvas.Visibility = Visibility.Visible;
        System.Windows.Ink.DrawingAttributes attributes = new System.Windows.Ink.DrawingAttributes();
        attributes.Color = m_structure.Color;
        m_inkCanvas.DefaultDrawingAttributes = attributes;
      }
      else
      {
        m_inkCanvas.Visibility = Visibility.Hidden;
      }
    }
    ...
    VMS.TPS.Common.Model.API.StructureSet m_structureSet;
    ...
    System.Windows.Controls.InkCanvas m_inkCanvas;
    System.Windows.Controls.ComboBox m_structuresCombo;
